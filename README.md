...
3. Use your browser or application with mitmproxy running (or configure your system to use the mitmproxy instance). The addon persists extracted tweets into SQLite (`mitm_twitter_search.db`) — it does not write extracted JSONL files.
...
Next steps
 - If you want, I can add filtering options, write unit tests, or provide helper scripts to query/export the SQLite database.
...
<!-- JSONL output removed: the addon writes only to SQLite. -->
